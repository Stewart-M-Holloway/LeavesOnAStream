import {
  ...
  getRotationalDrag,
  getTorque,
} from '../src/components/Common/LeafPhysics';
...
describe('Testing getXYVelocity', () => {
  ...
  test('Positive velocity results in positive x and y velocity', () => {
    expect(getXYVelocity(1, 1, [0, 0, 0], [0, 0, 0], 100)).toEqual({ xVel: 0.0025, yVel: 0.0025 });
  });
  test('Negative velocity results in negative x and y velocity', () => {
    expect(getXYVelocity(-1, -1, [0, 0, 0], [0, 0, 0], 100)).toEqual({
      xVel: -0.0025,
      yVel: -0.0025,
    });
  });
